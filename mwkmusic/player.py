...
async def play_track(client, m: Message):
    ...
    if len(playlist) == 1:
        m_status = await m.reply_text(
            f"{emoji.INBOX_TRAY} **Yup Bruh... Transcoding 😎.**"
        )
...
@Client.on_message(filters.command("ubjoin") & filters.user(ADMINS))
async def join_group_call(client, m: Message):
    group_call = mp.group_call
    if group_call.is_connected:
        await m.reply_text(f"{emoji.ROBOT} **My Userbot Already Joined To The Voice Chat!**")
        return
    await mp.start_call()
...
@Client.on_message(current_vc
                   & filters.command("leavevc") & filters.user(ADMINS))
